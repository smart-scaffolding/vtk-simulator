...
async def writer(iterations, hub, id):

    for x in range(iterations):
        print(f'Writer: I have {len(hub.subscriptions)} subscribers now')
        hub.publish(f'Hello world - {x} from {id}')
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(asyncio.gather(writer(10, hub, 1), *readers))
    loop.run_until_complete(asyncio.gather(writer(4, hub, 2), *readers))
